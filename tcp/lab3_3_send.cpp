...
const int N = 50;//GBN窗口大小
int cwnd, ssthresh;//拥塞控制窗口大小
...
void send_message(char* message, int message_length) {//发送message，其长度为message_length
    int package_num = message_length / Max_pack_len + (message_length % Max_pack_len != 0);//确定发包数，是否加最后一个包

    cwnd = 1;//初始拥塞窗口大小
    ssthresh = N;//初始拥塞阈值
    int dup_ack_count = 0;//收到重复ack个数
    int window_size = min(cwnd, N);//实际发送窗口大小取决于拥塞控制和流量控制的min
    ...
    while (base < package_num) {
        window_size = min(cwnd, N);
        printf("窗口初始包:%d\t窗口末尾包:%d\tcwnd大小:%d\tssthresh大小:%d\t\n", base, package_i, cwnd, ssthresh);
        //1. 发送第package_i到第package_i+window_size-1个包(将窗口发满)或将数据发完
        pre_seq = next_verified_seq;
        while (package_i - base < window_size && package_i < package_num) {
            std::printf("正在发送第%d/%d个数据包\n", package_i, package_num);
        ...
        while (recvfrom(c_socket, recv_buf, head_len, 0, (sockaddr*)&server_addr, &lentmp) == SOCKET_ERROR)
            if (clock() - timer_begin > TIMEOUT) {
                //2.1 超时，返回重传window_size个
                timeout = true;
                package_i = base;//重置发送位置为窗口首位置
                timer_begin = clock();//重新启动定时器
                printf("超时\n");
                //进入慢启动阶段，重置
                ssthresh = cwnd / 2;
                cwnd = 1;
                dup_ack_count = 0;//错误次数重置
                break;
            }
        if (timeout) continue;
        //move_num是收到的ack与窗口首位置的移动步数
        int move_num = (next_verified_seq > MAX_CHAR - N && recv_buf[3] < N) ?
            recv_buf[3] + MAX_CHAR - next_verified_seq :
            recv_buf[3] - next_verified_seq;
        move_num++;
        if (cksum(recv_buf, head_len) == 0 && (recv_buf[1] & FLAG_ACK) && move_num > 0) {
            //2.2 成功接收,窗口向前移动
            std::printf("接收新的ACK\n");//日志
            base += move_num;
            ...
            //2.3 未成功接收，计时器不变，返回重传（或等待）
            dup_ack_count++;
            std::printf("接收第%d次重复ACK\n", dup_ack_count);//日志
            // 若3次重复ack，进入快速恢复阶段
            if (dup_ack_count == 3) {
                ssthresh = cwnd / 2;
                cwnd = ssthresh + 3;
    ...
    pre_seq = next_verified_seq;//最后调整seq
}
